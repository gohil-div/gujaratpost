...
export default function NewsGallary({ customClass, dark, data }) {
      ...
      >
        {newsData?.map((item, i) => (
          <div
            ...
            key={i + 1}
          >
            <div className="bg-image" style={{backgroundImage:`https://www.gujaratpost.in/news/${item.NEWS_IMAGE}`}}></div>
            <div className="post__gallery_play_content">
              <div className="post-meta">
                ...
                <div className="meta-date">
                  <span>{item.NEWS_MODIFY_DATE}</span>
                </div>
              </div>
              <h2 className="title">
                <Link href="/post-details-three">{item.NEWS_TITLE}</Link>
              </h2>
              <p>{item.postDesc}</p>
            </div>
