...
export default function MostviewNews({ dark, data }) {
  const transformedData = data?.map((apiResponse,index) => ({
    news_id:apiResponse.NEWS_ID,
    id: index+1,
  ...
  const chunkedData = chunkArray(transformedData, 6);
  return (
    <>
    ...
    <Slider {...setting} className="trending-sidebar-slider">
      {chunkedData.map((chunk, chunkIndex) => (
        <div className="post_gallery_items" key={chunkIndex}>
          {chunk.map((item) => (
            <div
            ...
            >
              ...
              <div className="gallery_item_content">
                ...
                <h4 className="title">
                  <Link href={`/post-details-three/${item.news_id}`}>{item.postTitle}</Link>
                </h4>
                <span>{item.id}</span>
              </div>
            </div>
